...
void HUART_voidInit(u8 UART_CHANNEL)
{
	...
	GPIO_InitPins(&(HUART_cnfg[UART_CHANNEL].GPIO_Input));
	GPIO_InitPins(&(HUART_cnfg[UART_CHANNEL].GPIO_Output));
	...
	if (UART_cfg->RX_Mode==UART_MODE_DMA)
	{
		HRCC_EnablePeripheral(HRCC_PERIPHERAL_AHBENR_DMA1);
	...
	{
		UART_voidInitStruct(UART_CHANNEL,HRCC_GetAPB2_Speed());

	}
	else
	{
		UART_voidInitStruct(UART_CHANNEL,HRCC_GetAPB1_Speed());
